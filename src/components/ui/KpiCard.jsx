export default function KpiCard({ icon: Icon, label, value, helper, tone = 'cyan' }) {
  ...
  return (
    <div className={`metric-card min-h-[132px] ${toneClasses[tone] || toneClasses.cyan}`}>
      <div className="relative z-10 flex items-center gap-4">
        ...
        <div className="min-w-0">
          <strong className="sig-number block text-3xl leading-none text-white md:text-4xl">{value}</strong>
          <span className="mt-2 block text-[13px] font-semibold uppercase leading-snug tracking-[0.08em] text-slate-300">{label}</span>
          {helper && <small className="mt-1 block text-[13px] font-normal leading-snug text-slate-400">{helper}</small>}
        </div>
      </div>
    </div>
  );
